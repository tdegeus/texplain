...
"""
    ...
    for i in range(3):
        tex.format_labels()
        assert tex.labels() == expect
        assert formatted.strip() == str(tex).strip()


def test_format_labels_prefix():
    text = r"""
\chapter{My chapter}
\label{main}
...
def test_replace_command_simple():
    ...
    assert expect.strip() == str(tex).strip()
...
def test_replace_command_recursive():
...
def test_remove_comments_external():
    source = r"""
